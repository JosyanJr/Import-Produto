...
def gerar_sql_inserts(df):
    ...
    for index, row in df.iterrows():
        ...
        insert = f"insert into produto_empresa(empid, preid, proid, proCodigo, ProCusto, ProVenda, ProUn, ProUnTrib, proUnComercialId, proUnTribId, proCodCst2, proCodCSOSN, proListaInvent, proEstoqueMin) VALUES(1, {id}, {id}, {row['proCodigo']}, {row['ProCusto']}, {row['ProVenda']}, '{row['ProUn']}', '{row['ProUnTrib']}', {row['proUnComercialId']}, {row['proUnTribId']}, '{Cst2}', '{row['proCodCSOSN']}', '-1', 1) go"
        inserts.append(insert)
...
if __name__ == "__main__":
    ...
    df = pd.read_excel("C:\\Users\\Elson Jr\\Downloads\\Import-Produto-main\\dadosProdutos.xlsx", sheet_name='Plan1')
    ...
    arquivo = "C:\\Users\\Elson Jr\\Downloads\\Import-Produto-main\\insertsProdutos.txt"
